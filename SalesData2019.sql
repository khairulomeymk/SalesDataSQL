...
--revenure by month, state, city
SELECT MONTH(ORDER_DATE) AS 'MONTH', CITY, STATE, SUM(convert(int, [Quantity_Ordered])*(convert(int,[Price_Each]))) AS REVENUE 
FROM SALES_2019_ADDRESSUPDATED
GROUP BY MONTH(ORDER_DATE), CITY, STATE
ORDER BY REVENUE DESC
